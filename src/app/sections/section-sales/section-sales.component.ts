...
export class SectionSalesComponent implements OnInit {
  ...
  constructor(private salesDataSrvice: SalesDataService) { }

  ngOnInit(): void {
    this.salesDataSrvice.getOrdersByState().subscribe(res => {
      this.salesDataByState = res;
    });
    this.salesDataSrvice.getOrdersByCustomer(5).subscribe(res => {
